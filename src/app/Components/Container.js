...
import dat from "@/app/databasename.json"
...
const Container=() =>{
  ...
  const [cookies,setCookie,removeCookie]=useCookies(['temp']);
...
console.log("search",searchterm)
 ...
 const handleKeyDown = (event) => {
  if (event.key === 'Enter') {
    if (selectedIndex >= 0 && selectedIndex < filteredData.length) {
      onSearch(filteredData[selectedIndex].full_name);
 ...
 const Processing=()=>{
  ...
  let c=0;
  var m =require("@/app/Prices.json");
  for (let i=0;i<m.length;i++)
    {
      let obj =m[i];
      var p=obj.Model;
      if(p.localeCompare(value)==0)
        {
          c=c+1;
...
<path
        ...
        />
        <button required ="" type="submit" id="bid" className="relative p-2 bg-[#0d1829] rounded-full" onClick={Processing}>
      ...
      <div className="dropdown bg-[#d7dbe0] flex flex-col border border-gray-400 empty:border-none w-10/12">
        {filteredData.map((item, index) => (
          <div
            ...
            className={`dropdownrow cursor-pointer text-start mx-2 my-0 border-spacing-1 border-zinc-950 hover:bg-white ${selectedIndex === index ? 'bg-white' : ''}`}
            key={item.full_name}
          >
            {item.full_name}
          </div>
